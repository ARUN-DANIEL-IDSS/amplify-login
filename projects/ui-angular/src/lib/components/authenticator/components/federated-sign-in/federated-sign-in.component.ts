...
export class FederatedSignInComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.includeAmazon = socialProviders?.includes('amazon') ?? false;
    this.includeApple = socialProviders?.includes('apple') ?? false;
    this.includeGoogle = socialProviders?.includes('google') ?? false;
    this.includeFacebook = socialProviders?.includes('facebook') ?? false;
